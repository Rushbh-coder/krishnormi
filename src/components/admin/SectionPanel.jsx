...
export default function SectionPanel({ title, description, visible, savedVisible, onVisibleChange, lastSaved, children }) {
  const statusVisible = savedVisible ?? visible;
        ...
        {onVisibleChange && (
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-[7px] font-heading text-xs font-semibold ${
              statusVisible ? 'border-[#d6ebe1] bg-[#edf7f2] text-[#0b6b45]' : 'border-[#e4e7e5] bg-[#f5f6f5] text-[#7a8d87]'
            }`}
          >
            <span className={`h-1.5 w-1.5 rounded-full ${statusVisible ? 'bg-[#0b6b45]' : 'bg-[#a1aea9]'}`} aria-hidden="true" />
