...
function Card({ card, template, visible, delay }) {
  const staggerStyle = { transitionDelay: visible ? `${delay}ms` : "0ms" };
  const staggerClass = `transition-[opacity,transform] duration-700 ease-out motion-reduce:transition-none ${
    visible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
  }`;

  if (card.type === "image") {
    return (
      <div style={staggerStyle} className={`overflow-hidden rounded-[10px] max-[640px]:h-[240px] ${staggerClass}`}>
        <img
  ...
  return (
    <div
      style={staggerStyle}
      className={`relative flex flex-col overflow-hidden rounded-[10px] p-[27px_29px] shadow-[0px_2px_80px_0px_rgba(0,0,0,0.1)] ${staggerClass} ${
    ...
    >
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <h3
          className={`m-0 font-heading text-xl leading-[1.4] font-semibold ${
      ...
      {/* Static Full Text */}
      <p
        className={`m-0 font-heading text-[15px] leading-[1.85] ${
  ...
  return (
    <section className="bg-tint py-[90px]">
      <div className="container">
        ...
        <div
          ref={gridRef}
          className="grid grid-cols-4 auto-rows-[minmax(278px,auto)] gap-6 max-[1100px]:grid-cols-2 max-[640px]:grid-cols-1"
        >
          {cards.map((card, i) => (
            <Card key={i} card={card} template={TEMPLATE[i] ?? {}} visible={gridVisible} delay={i * 80} />
          ))}
        </div>
      </div>
    </section>
